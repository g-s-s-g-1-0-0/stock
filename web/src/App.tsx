    ...
    '2': [
      {
        ...
        value: [
          '전략 1 시즌 열림',
          '회복장',
          'QQQ 이격도 ≤ 매수 차단선 (회복장 +18% / 정상장·횡보 고점 +9%)',
      ...
      },
      {
        ...
        value: [
          ...
          'QQQ 이격도는 나스닥이 장기 평균(MA200)보다 얼마나 위에 있는지 보는 값입니다.',
          '매수 차단선은 회복장이면 +18%, 그 외(정상장·횡보 고점)면 +9%입니다. 이 선을 넘으면 신규·추가 매수를 막습니다.',
...
const valueMetricColumns: Array<{ label: string; value: (metric: ValuationMetric) => string; tooltip?: string }> = [
  { label: 'Market Cap', value: (metric) => metric.marketCap, tooltip: metricTooltip('회사의 전체 몸값입니다. 큰 회사일수록 안정적일 수 있지만, 같은 업종 대비 너무 비싼지는 함께 봅니다.', '10조 이상 긍정, 1조 이상 일반, 그 미만 부정.') },
  { label: 'Sales', value: (metric) => metric.sales, tooltip: metricTooltip('최근에 벌어들인 매출 규모입니다. 매출이 크더라도 성장률이 낮으면 투자 매력은 줄 수 있습니다.', '1조 이상 긍정, 1,000억 이상 일반, 그 미만 부정.') },
  { label: 'Sales Q/Q', value: (metric) => metric.salesQoq, tooltip: metricTooltip('직전 분기보다 매출이 얼마나 늘었는지 봅니다. 높으면 최근 흐름이 좋고, 계속 마이너스면 수요 둔화를 의심합니다.', '10% 이상 긍정, 0% 이상 일반, 그 미만 부정.') },
  { label: 'Sales Y/Y (TTM)', value: (metric) => metric.salesYoyTtm, tooltip: metricTooltip('최근 12개월 매출이 1년 전보다 얼마나 늘었는지 봅니다. 성장률이 둔화되면 비싼 가격을 조심해서 봅니다.', '15% 이상 긍정, 0% 이상 일반, 그 미만 부정.') },
  { label: 'Sales past 3/5Y', value: (metric) => metric.salesPastYears, tooltip: metricTooltip('최근 3년/5년 동안 매출이 꾸준히 늘었는지 봅니다. 들쭉날쭉하면 경기 영향을 많이 받는지 확인합니다.', '3·5년 평균 12% 이상 긍정, 0% 이상 일반, 그 미만 부정.') },
  { label: 'Current Ratio', value: (metric) => metric.currentRatio, tooltip: metricTooltip('1년 안에 갚을 돈을 감당할 여력이 있는지 봅니다. 보통 100% 이상이면 단기 자금 사정이 무난하다고 봅니다.', '120% 이상 긍정, 70% 이상 일반, 그 미만 부정.') },
  { label: 'P/FCF', value: (metric) => metric.priceToFreeCashFlow, tooltip: metricTooltip('회사가 실제로 남기는 현금 대비 가격입니다. 낮으면 현금창출력 대비 싸고, 높으면 기대가 많이 반영된 상태일 수 있습니다.', '20 이하 긍정, 35 이하 일반, 그 초과 부정.') },
  { label: 'P/S', value: (metric) => metric.priceToSales, tooltip: metricTooltip('매출 대비 회사 가격이 얼마나 비싼지 봅니다. 낮을수록 부담이 작고, 성장주는 업종 평균과 같이 비교합니다.', '3 이하 긍정, 8 이하 일반, 그 초과 부정.') },
  { label: 'PER', value: (metric) => metric.per, tooltip: metricTooltip('이익 대비 주가가 얼마나 비싼지 보는 지표입니다. 낮으면 싸 보일 수 있고, 성장률이 낮은데 높으면 부담입니다.', '25 이하 긍정, 40 이하 일반, 그 초과 부정.') },
  { label: 'PBR', value: (metric) => metric.pbr, tooltip: metricTooltip('회사가 가진 순자산 대비 주가가 비싼지 봅니다. 수익성이 낮은데 이 값이 높으면 주의가 필요합니다.', '3 이하 긍정, 8 이하 일반, 그 초과 부정.') },
  { label: 'ROE', value: (metric) => metric.roe, tooltip: metricTooltip('회사가 가진 돈으로 얼마나 이익을 잘 내는지 봅니다. 높고 꾸준하면 좋지만, 빚 때문에 높아진 건 아닌지 확인합니다.', '20% 이상 긍정, 10% 이상 일반, 그 미만 부정.') },
  { label: 'PEG', value: (metric) => metric.peg, tooltip: metricTooltip('이익 성장 속도 대비 주가가 비싼지 봅니다. 1 안팎이면 무난하고, 높을수록 성장 대비 비싸다는 뜻입니다.', '1.2 이하 긍정, 2 이하 일반, 그 초과 부정.') },
  ...
  { label: 'Gross Margin', value: (metric) => metric.grossMargin, tooltip: metricTooltip('제품을 팔고 원가를 뺀 뒤 얼마나 남는지 봅니다. 높을수록 가격 경쟁력이 좋고, 하락하면 원가 부담을 의심합니다.', '50% 이상 긍정, 25% 이상 일반, 그 미만 부정.') },
  { label: 'Oper. Margin', value: (metric) => metric.operatingMargin, tooltip: metricTooltip('본업에서 매출 대비 얼마나 이익을 남기는지 봅니다. 높고 안정적이면 좋고, 마이너스면 비용 구조를 먼저 봅니다.', '20% 이상 긍정, 8% 이상 일반, 그 미만 부정.') },
  { label: 'EPS (TTM)', value: (metric) => metric.epsTtm, tooltip: metricTooltip('최근 12개월 동안 주식 1주당 벌어들인 이익입니다. 높고 증가하면 이익 체력이 좋다고 봅니다.', '플러스 긍정, 0 일반, 마이너스 부정.') },
  { label: 'EPS Next Y', value: (metric) => metric.epsNextYear, tooltip: metricTooltip('다음 해에 예상되는 1주당 이익입니다. 현재보다 높으면 성장 기대가 있고, 자주 낮아지면 보수적으로 봅니다.', '플러스 긍정, 0 일반, 마이너스 부정.') },
  { label: 'EPS Q/Q (%)', value: (metric) => metric.epsQoq, tooltip: metricTooltip('직전 분기보다 1주당 이익이 얼마나 늘었는지 봅니다. 높으면 최근 실적 흐름이 좋다는 뜻입니다.', '20% 이상 긍정, 0% 이상 일반, 그 미만 부정.') },
  { label: 'Rule of 40%', value: (metric) => metric.ruleOf40, tooltip: metricTooltip('성장률과 이익률을 같이 보는 지표입니다. 40% 이상이면 성장과 수익의 균형이 좋다고 봅니다.', '40% 이상 긍정, 20% 이상 일반, 그 미만 부정.') },
  { label: '실적발표일 (한국 시간 기준)', value: (metric) => metric.earningsDate, tooltip: metricTooltip('한국 시간 기준 실적 발표일입니다. 실적 전후에는 가격이 크게 움직일 수 있어 주의합니다.', '당일(D-0) 주의(빨강), D-1~D-3 임박(주황), 그 외 없음.') },
]
...
const RSI_TONE_GUIDE = '35 이하 또는 45~80 긍정, 35 초과~45 미만 일반, 80 초과 부정.'
const CCI_TONE_GUIDE = '-150 이하 긍정, 200 이하 일반, 그 초과 부정.'
const MACD_TONE_GUIDE = '0 이상 긍정, -500 이상 일반, 그 미만 부정.'
const MACD_HIST_TONE_GUIDE = '0 이상 긍정, -1 이상 일반, 그 미만 부정.'
const ADX_TONE_GUIDE = '30 이상 긍정, 20 이상 일반, 그 미만 부정.'
const VOLUME_RATIO_TONE_GUIDE = '100~200% 긍정, 50~250% 일반, 그 외 부정.'
const BB_HIGH_TONE_GUIDE = '35~85 긍정, 20~95 일반, 그 외 부정.'
const BB_WIDTH_TONE_GUIDE = '60일 평균의 85% 이하 긍정, 120% 이하 일반, 그 초과 부정.'
const MA_DEFAULT_TONE_GUIDE = '현재가가 선 위~+80% 긍정, -5%~+100% 일반, 그 외 부정.'
const EARNINGS_TONE_GUIDE = '당일(D-0) 주의(빨강), D-1~D-3 임박(주황), 그 외 없음.'
...
const technicalMetricColumns: TechnicalColumn[] = [
  ...
  { key: 'RSI 기울기', label: 'RSI 변화(D-D-1)', tooltip: metricTooltip('오늘 RSI에서 어제 RSI를 뺀 값입니다. 플러스면 매수세가 강해지고, 마이너스면 힘이 약해지는 흐름입니다.', '+2 이상 긍정, 0 이상 일반, 그 미만 부정.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 4, -9, 16), 2) },
  ...
  { key: 'CCI 기울기', label: 'CCI 변화(D-D-1)', tooltip: metricTooltip('오늘 CCI에서 어제 CCI를 뺀 값입니다. 크게 오르면 반등 시도, 크게 내리면 힘이 약해진 흐름입니다.', '+10 이상 긍정, 0 이상 일반, 그 미만 부정.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 8, -58, 136), 2) },
  ...
  { label: '+DI (DMI, 14)', tooltip: metricTooltip('상승 힘을 보여주는 지표입니다. +DI가 -DI보다 높으면 매수세가 더 강하다고 봅니다.', '-DI보다 3 이상 크면 긍정, ±3 이내 일반, 그 외 부정.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 16, 12, 52), 2) },
  { label: '-DI (DMI, 14)', tooltip: metricTooltip('하락 힘을 보여주는 지표입니다. -DI가 +DI보다 높으면 매도 압력이 더 강하다고 봅니다.', '+DI보다 3 이상 작으면 긍정, ±3 이내 일반, 그 외 부정.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 17, 9, 48), 2) },
  ...
  { key: 'ADX 기울기', label: 'ADX 변화(D-D-1)', tooltip: metricTooltip('오늘 ADX에서 어제 ADX를 뺀 값입니다. 오르면 추세가 강해지고, 내리면 횡보 가능성이 커집니다.', '+1 이상 긍정, 0 이상 일반, 그 미만 부정.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 21, -6, 12), 2) },
  { key: 'Candle Open', label: '시가(D)', tooltip: metricTooltip('오늘 장이 시작된 가격입니다. 종가와 비교해 장중에 매수세가 강했는지 봅니다.', '종가가 시가보다 +2% 이상 긍정, ±2% 이내 일반, 그 외 부정.'), value: (stock, index) => formatTechnicalPrice(stock, stockPriceNumber(stock) * technicalNumber(stock, index, 22, 0.965, 0.07, 4)) },
  ...
  { label: '아래꼬리 길이', tooltip: metricTooltip('시가와 종가 중 낮은 값에서 저가를 뺀 폭을 종가 대비 비율로 봅니다. 길수록 저점 매수세가 들어왔다고 볼 수 있습니다.', '1.5% 이상 긍정, 0.5% 이상 일반, 그 미만 부정.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 26, 0, 4), 2)}%` },
  { label: '위꼬리 길이', tooltip: metricTooltip('고가에서 시가와 종가 중 높은 값을 뺀 폭을 종가 대비 비율로 봅니다. 길수록 위에서 매물이 많이 나왔다고 볼 수 있습니다.', '1% 이하 긍정, 2.5% 이하 일반, 그 초과 부정.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 27, 0, 4), 2)}%` },
  { label: '몸통 길이', tooltip: metricTooltip('시가와 종가의 차이를 종가 대비 비율로 봅니다. 클수록 그날 방향성이 뚜렷합니다.', '1% 이상 긍정, 0.3% 이상 일반, 그 미만 부정.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 28, 0.05, 5), 2)}%` },
  ...
  { label: '볼린저밴드 %B (종가)', tooltip: metricTooltip('종가가 가격 범위 안에서 위쪽인지 아래쪽인지 봅니다. 80 이상은 상단, 20 이하는 하단에 가깝습니다.', '30~85 긍정, 20~95 일반, 그 외 부정.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 33, 5, 112), 2) },
  { label: '볼린저밴드 %B (저가)', tooltip: metricTooltip('오늘 저가가 가격 범위 안에서 어디였는지 봅니다. 장중에 아래쪽을 찍고 회복했는지 확인합니다.', '50 이하 긍정, 80 이하 일반, 그 초과 부정.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 34, 0, 105), 2) },
  ...
  { label: '20일 이동평균선', tooltip: metricTooltip('최근 20일 평균 가격입니다. 이 선 위에 있으면 단기 상승 흐름이 유지된다고 봅니다.', '현재가가 선 위~+8% 긍정, -2%~+20% 일반, 그 외 부정.'), value: (stock, index) => formatTechnicalPrice(stock, stockPriceNumber(stock) * technicalNumber(stock, index, 41, 0.92, 0.13, 4)) },
  ...
  { label: '200일 이동평균선', tooltip: metricTooltip('최근 200일 평균 가격입니다. 현재가가 이 선 위면 장기 흐름이 좋다고 보는 경우가 많습니다.', '현재가가 선 위~+80% 긍정, -10%~+100% 일반, 그 외 부정.'), value: (stock, index) => formatTechnicalPrice(stock, stockPriceNumber(stock) * technicalNumber(stock, index, 44, 0.72, 0.28, 4)) },
  ...
  { key: 'ATR (14, %)', label: '하루 평균 변동폭(ATR%)', tooltip: metricTooltip('이 종목이 최근 14일 동안 하루에 평균 몇 % 움직였는지 봅니다. 값이 클수록 크게 출렁이는 종목이라 손절 여유를 더 두거나 수량을 줄이는 편이 안전합니다.', '3% 이하 긍정, 6% 이하 일반, 그 초과 부정.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 46, 1.5, 5), 2)}%` },
  { key: '52주 신고가 대비', label: '52주 신고가 대비', tooltip: metricTooltip('최근 1년 최고가에서 지금 얼마나 내려와 있는지 봅니다. 0%에 가까울수록 신고가 부근의 강한 종목이고, -30% 이하면 고점에서 크게 밀린 상태입니다.', '-8% 이상 긍정, -25% 이상 일반, 그 미만 부정.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 47, -38, 38), 2) + '%' },
  { key: '52주 신고가 후 경과일', label: '신고가 후 경과일', tooltip: metricTooltip('최근 1년 최고가를 찍은 뒤 며칠이 지났는지 봅니다. 짧을수록 최근까지 신고가를 만들던 강한 흐름입니다.', '20일 이하 긍정, 60일 이하 일반, 그 초과 부정.'), value: (stock, index) => `${Math.round(technicalNumber(stock, index, 48, 1, 180, 0))}일` },
  { key: 'QQQ 대비 상대강도 (20일)', label: 'QQQ 대비 상대강도(20일)', tooltip: metricTooltip('최근 20거래일 동안 이 종목이 나스닥(QQQ)보다 얼마나 더 올랐는지(%p)입니다. +면 시장보다 강함, -면 약함.', '+2%p 이상 긍정, -3%p 이상 일반, 그 미만 부정.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 49, -12, 24), 2) + '%p' },
