...
function watchlistByTypeFromCanonical(tickers: string[]): Record<InvestmentType, string[]> {
  return {
    long_term: tickers,
    swing: tickers,
  }
...
// 활성 투자 유형의 목록만 새 값으로 교체하고 상대 유형 목록은 그대로 보존한다.
// 가치투자/스윙투자가 서로 독립된 관심종목을 유지하도록 하는 핵심 헬퍼다.
function watchlistByTypeWithActive(
  base: Record<InvestmentType, string[]>,
  activeType: InvestmentType,
  tickers: string[],
): Record<InvestmentType, string[]> {
  return {
    long_term: activeType === 'long_term' ? tickers : base.long_term,
    swing: activeType === 'swing' ? tickers : base.swing,
  }
}
...
function strategyInfo(strategy: string, investmentType: InvestmentType = 'swing') {
  ...
  const descriptions: Record<InvestmentType, Record<string, string>> = {
    swing: {
      ...
      '7': '20·50·120·200일 이동평균선 중 하나를 터치한 뒤 선 위로 반등하는 종목을 매수하는 스윙 전략입니다. 고정 익절 없이 회복장 종료·나스닥 고점 알람으로 청산하고, 진입가 대비 -8% 손절을 적용합니다.',
    ...
    '1': [
      {
        label: '진입',
        value: [
          'QQQ 현재가가 200일 평균선보다 3% 넘게 아래',
          '종목 현재가가 200일 평균선(MA200) 아래',
          'VIX(공포지수) 22 이상',
          'RSI 35 미만 또는 CCI -150 미만',
          'LR 추세선 방어',
        ],
      },
      {
        ...
        value: [
          ...
          'QQQ 현재가가 200일 평균선보다 얼마나 위/아래에 있는지를 “이격도”라고 부릅니다. -3%면 1년 평균 가격보다 3% 싸다는 뜻이고, 그보다 더 아래면 시장 전체가 눌린 상태로 봅니다.',
      ...
      },
      {
        label: '관망',
        value: [
          '보유 중에도 “아직 공포 저점 자리인가”를 진입 때와 같은 조건으로 다시 봅니다. 조건이 깨지면 매수 → 관망으로 바뀌며, 자동으로 파는 신호는 아닙니다.',
          '공포가 살짝만 줄었다고 바로 관망으로 바꾸지 않기 위해, 공포지수(VIX)만은 22가 아니라 18 이상이면 매수 의견을 유지합니다.',
        ],
      },
      {
        label: '재진입',
    ...
    '2': [
      {
        ...
        value: [
          '전략 1 시즌 열림',
          '회복장',
          'QQQ가 200일 평균선 대비 매수 차단선 아래 (회복장 +18% / 그 외 +9%)',
      ...
      },
      {
        ...
        value: [
          ...
          '“이격도”는 QQQ 현재가가 200일 평균선(최근 약 1년 평균 가격)보다 몇 % 위/아래인지를 뜻합니다. +9%면 1년 평균보다 9% 비싸진 상태입니다.',
          '매수 차단선은 회복장이면 +18%, 그 외(정상장·횡보 고점)면 +9%입니다. 이 선을 넘으면 너무 비싸진 것으로 보고 신규·추가 매수를 막습니다.',
    ...
    '3': [
      {
        label: '진입',
        value: [
          'QQQ가 200일 평균선 대비 -3% ~ +7% (정상장 안의 완충 구간)',
          '종목 현재가가 200일 평균선(MA200) 위',
          '저가 기준 볼린저 %B ≤ 10',
          'RSI 45 이하',
          '같은 날 전략 1·2 미충족',
        ],
      },
      {
        label: '설명',
        value: [
          'QQQ 이격도 -3% ~ +9%는 “정상장”입니다. 전략 3은 그중에서도 +7% 이하에서만 새로 진입해, +9% 경계 근처의 흔들림을 피합니다.',
          '-3%보다 아래면 하락장, +9%보다 위면 횡보장 고점, 급락 뒤 다시 올라온 구간이면 회복장으로 따로 봅니다.',
          '종목은 MA200(최근 약 1년 평균 가격) 위에 있어야 장기 추세가 무너지지 않은 것으로 봅니다.',
          '볼린저 밴드는 최근 가격 변동폭으로 그린 위·아래 띠입니다. 저가 %B ≤ 10은 당일 저가가 그 아래 띠 근처까지 내려왔다는 뜻입니다.',
          'RSI 45 이하는 단기 과열이 아닌 눌림 상태를 뜻합니다.',
      ...
      {
        label: '손절',
        value: '진입 당시 매수가 아래에서 가장 가까운 지지선(BB 하단·중단 또는 MA5/20/60/120/144/200)에 ATR·최소폭 버퍼를 뺀 가격 아래로 일봉 종가가 마감하면 손절합니다. 단, -25% 하드 서킷브레이커는 즉시 적용합니다.',
      },
      {
        label: '청산',
        value: [
      ...
      },
      {
        label: '관망',
        value: [
          '보유 중에는 QQQ 이격도 -3% ~ +7%와 현재가 MA200 위 조건을 보면 매수 의견을 유지합니다. 이 조건이 풀리면 관망으로 바뀌며 즉시 매도는 아닙니다.',
  ...
  return (
    <>
      ...
      <div className="strategy-criteria-intro">
        <p>
          몇 년간 여러 전략을 테스트한 결과, 아래 전략이 <strong>리스크를 줄이면서 수익을 극대화</strong>하는 데 가장 좋은 전략이라고 판단합니다. 다만, 매수 기회가 드물고 기다리는 기간이 길 수 있기 때문에 다소 지루할 수 있습니다. 기다리다 지쳐 조급하게 매수하게 될 것 같다면, 대기 자금은 같은 증권 계좌의 <strong>원화·외화 RP</strong>에 넣어 이자를 받는 편이 나을 수 있습니다. RP에도 리스크는 있지만 주식보다는 낮은 편입니다. RP 투자 방법은 <strong>「OO증권 RP 투자」</strong>로 검색해 이용 중인 증권사의 안내를 참고하세요.
        </p>
      </div>
...
function marketEventDisplayLabel(value: string) {
  return marketEventDisplayLabels[value] ?? value
}
...
const valueMetricColumns: Array<{ label: string; value: (metric: ValuationMetric) => string; tooltip?: string }> = [
  ...
  { label: 'Sales Q/Q', value: (metric) => metric.salesQoq, tooltip: metricTooltip('직전 분기보다 매출이 얼마나 늘었는지 봅니다. 높으면 최근 흐름이 좋고, 계속 마이너스면 수요 둔화를 의심합니다.', '긍정: +10%↑ 성장 가속 · 일반: 0%↑ 유지 · 부정: 감소·수요 둔화.') },
  { label: 'Sales Y/Y (TTM)', value: (metric) => metric.salesYoyTtm, tooltip: metricTooltip('최근 12개월 매출이 1년 전보다 얼마나 늘었는지 봅니다. 성장률이 둔화되면 비싼 가격을 조심해서 봅니다.', '긍정: +15%↑ 연간 성장 · 일반: 0%↑ · 부정: 역성장.') },
  { label: 'Sales past 3/5Y', value: (metric) => metric.salesPastYears, tooltip: metricTooltip('최근 3년/5년 동안 매출이 꾸준히 늘었는지 봅니다. 들쭉날쭉하면 경기 영향을 많이 받는지 확인합니다.', '긍정: 평균 +12%↑ 장기 성장 · 일반: 0%↑ · 부정: 장기 역성장.') },
  { label: 'Current Ratio', value: (metric) => metric.currentRatio, tooltip: metricTooltip('1년 안에 갚을 돈을 감당할 여력이 있는지 봅니다. 보통 100% 이상이면 단기 자금 사정이 무난하다고 봅니다.', '긍정: 120%↑ 단기 여유 · 일반: 70%↑ · 부정: 미만 자금 압박.') },
  { label: 'P/FCF', value: (metric) => metric.priceToFreeCashFlow, tooltip: metricTooltip('회사가 실제로 남기는 현금 대비 가격입니다. 낮으면 현금창출력 대비 싸고, 높으면 기대가 많이 반영된 상태일 수 있습니다.', '긍정: ≤20 현금 대비 저평가 · 일반: ≤35 · 부정: 초과 기대 과다.') },
  { label: 'P/S', value: (metric) => metric.priceToSales, tooltip: metricTooltip('매출 대비 회사 가격이 얼마나 비싼지 봅니다. 낮을수록 부담이 작고, 성장주는 업종 평균과 같이 비교합니다.', '긍정: ≤3 매출 대비 저렴 · 일반: ≤8 · 부정: 초과 부담.') },
  { label: 'PER', value: (metric) => metric.per, tooltip: metricTooltip('이익 대비 주가가 얼마나 비싼지 보는 지표입니다. 낮으면 싸 보일 수 있고, 성장률이 낮은데 높으면 부담입니다.', '긍정: ≤25 이익 대비 여유 · 일반: ≤40 · 부정: 초과 고평가.') },
  { label: 'PBR', value: (metric) => metric.pbr, tooltip: metricTooltip('회사가 가진 순자산 대비 주가가 비싼지 봅니다. 수익성이 낮은데 이 값이 높으면 주의가 필요합니다.', '긍정: ≤3 자산 대비 여유 · 일반: ≤8 · 부정: 초과 프리미엄.') },
  { label: 'ROE', value: (metric) => metric.roe, tooltip: metricTooltip('회사가 가진 돈으로 얼마나 이익을 잘 내는지 봅니다. 높고 꾸준하면 좋지만, 빚 때문에 높아진 건 아닌지 확인합니다.', '긍정: 20%↑ 자본 효율 · 일반: 10%↑ · 부정: 미만 수익성 약함.') },
  { label: 'PEG', value: (metric) => metric.peg, tooltip: metricTooltip('이익 성장 속도 대비 주가가 비싼지 봅니다. 1 안팎이면 무난하고, 높을수록 성장 대비 비싸다는 뜻입니다.', '긍정: ≤1.2 성장 대비 합리적 · 일반: ≤2 · 부정: 초과 비쌈.') },
  ...
  { label: 'Gross Margin', value: (metric) => metric.grossMargin, tooltip: metricTooltip('제품을 팔고 원가를 뺀 뒤 얼마나 남는지 봅니다. 높을수록 가격 경쟁력이 좋고, 하락하면 원가 부담을 의심합니다.', '긍정: 50%↑ 원가 경쟁력 · 일반: 25%↑ · 부정: 미만 마진 약함.') },
  { label: 'Oper. Margin', value: (metric) => metric.operatingMargin, tooltip: metricTooltip('본업에서 매출 대비 얼마나 이익을 남기는지 봅니다. 높고 안정적이면 좋고, 마이너스면 비용 구조를 먼저 봅니다.', '긍정: 20%↑ 본업 수익 · 일반: 8%↑ · 부정: 미만·적자.') },
  { label: 'EPS (TTM)', value: (metric) => metric.epsTtm, tooltip: metricTooltip('최근 12개월 동안 주식 1주당 벌어들인 이익입니다. 높고 증가하면 이익 체력이 좋다고 봅니다.', '긍정: 플러스 흑자 · 일반: 0 · 부정: 마이너스 적자.') },
...
const technicalSummaryTooltips: Record<string, string> = {
  ...
  '장 상태': 'QQQ 현재가의 200일선 이격도와 최근 60거래일 이력으로 구분합니다.\n\n회복장: 최근 60거래일 최저 이격도 ≤ -5%, 현재 이격도 ≥ 0%\n하락장: 현재 이격도 < -3%\n정상장: -3% 이상 ~ +9% 이하 (회복장 제외)\n횡보장 고점: 현재 이격도 > +9% (회복장 제외)\n\n신규·추가 매수 차단선은 회복장 +18% 초과, 그 외 국면 +9% 초과입니다.',
...
const RSI_TONE_GUIDE = '긍정: ≤35 과매도·저점 후보 / 45~80 추세 유지 · 일반: 35~45 애매 · 부정: >80 과열.'
const CCI_TONE_GUIDE = '긍정: ≤-150 과매도·반등 후보 · 일반: ≤200 보통 · 부정: >200 과열.'
const MACD_TONE_GUIDE = '긍정: ≥0 상승 우위 · 일반: ≥-500 약세 완화 · 부정: <-500 하락 우위.'
const MACD_HIST_TONE_GUIDE = '긍정: ≥0 모멘텀 유지 · 일반: ≥-1 약화 초입 · 부정: <-1 모멘텀 약화.'
const ADX_TONE_GUIDE = '긍정: ≥30 추세 뚜렷 · 일반: ≥20 추세 형성 · 부정: <20 횡보.'
const VOLUME_RATIO_TONE_GUIDE = '긍정: 100~200% 관심 증가 · 일반: 50~250% 허용 · 부정: 너무 적거나 과열.'
const BB_HIGH_TONE_GUIDE = '긍정: 35~85 건강한 상단 · 일반: 20~95 · 부정: 극단 과열/침체.'
const BB_WIDTH_TONE_GUIDE = '긍정: 평균 85%↓ 압축·돌파 전 · 일반: 120%↓ · 부정: 초과 변동성 과다.'
const MA_DEFAULT_TONE_GUIDE = '긍정: 선 위~+80% 추세 위 · 일반: -5%~+100% · 부정: 이탈·과열.'
const EARNINGS_TONE_GUIDE = '당일(D-0) 주의(빨강·변동성 큼), D-1~D-3 임박(주황), 그 외 없음.'
...
const technicalMetricColumns: TechnicalColumn[] = [
  ...
  { key: 'RSI 기울기', label: 'RSI 변화(D-D-1)', tooltip: metricTooltip('오늘 RSI에서 어제 RSI를 뺀 값입니다. 플러스면 매수세가 강해지고, 마이너스면 힘이 약해지는 흐름입니다.', '긍정: +2↑ 매수세 강화 · 일반: 0↑ · 부정: 하락·힘 약화.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 4, -9, 16), 2) },
  ...
  { key: 'CCI 기울기', label: 'CCI 변화(D-D-1)', tooltip: metricTooltip('오늘 CCI에서 어제 CCI를 뺀 값입니다. 크게 오르면 반등 시도, 크게 내리면 힘이 약해진 흐름입니다.', '긍정: +10↑ 반등 힘 · 일반: 0↑ · 부정: 하락·약세.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 8, -58, 136), 2) },
  ...
  { label: '+DI (DMI, 14)', tooltip: metricTooltip('상승 힘을 보여주는 지표입니다. +DI가 -DI보다 높으면 매수세가 더 강하다고 봅니다.', '긍정: -DI보다 +3↑ 매수세 우위 · 일반: ±3 이내 · 부정: 매도세 우위.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 16, 12, 52), 2) },
  { label: '-DI (DMI, 14)', tooltip: metricTooltip('하락 힘을 보여주는 지표입니다. -DI가 +DI보다 높으면 매도 압력이 더 강하다고 봅니다.', '긍정: +DI보다 -3↓ 매도 압력 약함 · 일반: ±3 이내 · 부정: 매도 압력 강함.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 17, 9, 48), 2) },
  ...
  { key: 'ADX 기울기', label: 'ADX 변화(D-D-1)', tooltip: metricTooltip('오늘 ADX에서 어제 ADX를 뺀 값입니다. 오르면 추세가 강해지고, 내리면 횡보 가능성이 커집니다.', '긍정: +1↑ 추세 강화 · 일반: 0↑ · 부정: 약화·횡보.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 21, -6, 12), 2) },
  { key: 'Candle Open', label: '시가(D)', tooltip: metricTooltip('오늘 장이 시작된 가격입니다. 종가와 비교해 장중에 매수세가 강했는지 봅니다.', '긍정: 종가>+2% 장중 매수세 · 일반: ±2% · 부정: 종가 약세.'), value: (stock, index) => formatTechnicalPrice(stock, stockPriceNumber(stock) * technicalNumber(stock, index, 22, 0.965, 0.07, 4)) },
  ...
  { label: '아래꼬리 길이', tooltip: metricTooltip('시가와 종가 중 낮은 값에서 저가를 뺀 폭을 종가 대비 비율로 봅니다. 길수록 저점 매수세가 들어왔다고 볼 수 있습니다.', '긍정: 1.5%↑ 저점 지지 · 일반: 0.5%↑ · 부정: 지지 약함.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 26, 0, 4), 2)}%` },
  { label: '위꼬리 길이', tooltip: metricTooltip('고가에서 시가와 종가 중 높은 값을 뺀 폭을 종가 대비 비율로 봅니다. 길수록 위에서 매물이 많이 나왔다고 볼 수 있습니다.', '긍정: ≤1% 매물 적음 · 일반: ≤2.5% · 부정: 초과 매도 압력.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 27, 0, 4), 2)}%` },
  { label: '몸통 길이', tooltip: metricTooltip('시가와 종가의 차이를 종가 대비 비율로 봅니다. 클수록 그날 방향성이 뚜렷합니다.', '긍정: 1%↑ 방향성 뚜렷 · 일반: 0.3%↑ · 부정: 횡보·애매.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 28, 0.05, 5), 2)}%` },
  ...
  { label: '볼린저밴드 %B (종가)', tooltip: metricTooltip('종가가 가격 범위 안에서 위쪽인지 아래쪽인지 봅니다. 80 이상은 상단, 20 이하는 하단에 가깝습니다.', '긍정: 30~85 건강한 위치 · 일반: 20~95 · 부정: 극단 과열/침체.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 33, 5, 112), 2) },
  { label: '볼린저밴드 %B (저가)', tooltip: metricTooltip('오늘 저가가 가격 범위 안에서 어디였는지 봅니다. 장중에 아래쪽을 찍고 회복했는지 확인합니다.', '긍정: ≤50 눌림 후 회복 후보 · 일반: ≤80 · 부정: 상단만 터치·과열.'), value: (stock, index) => formatTechnicalNumber(technicalNumber(stock, index, 34, 0, 105), 2) },
  ...
  { label: '20일 이동평균선', tooltip: metricTooltip('최근 20일 평균 가격입니다. 이 선 위에 있으면 단기 상승 흐름이 유지된다고 봅니다.', '긍정: 선 위~+8% 단기 추세 · 일반: -2%~+20% · 부정: 이탈·과열.'), value: (stock, index) => formatTechnicalPrice(stock, stockPriceNumber(stock) * technicalNumber(stock, index, 41, 0.92, 0.13, 4)) },
  ...
  { label: '200일 이동평균선', tooltip: metricTooltip('최근 200일 평균 가격입니다. 현재가가 이 선 위면 장기 흐름이 좋다고 보는 경우가 많습니다.', '긍정: 선 위~+80% 장기 추세 · 일반: -10%~+100% · 부정: 이탈·과열.'), value: (stock, index) => formatTechnicalPrice(stock, stockPriceNumber(stock) * technicalNumber(stock, index, 44, 0.72, 0.28, 4)) },
  ...
  { key: 'ATR (14, %)', label: '하루 평균 변동폭(ATR%)', tooltip: metricTooltip('이 종목이 최근 14일 동안 하루에 평균 몇 % 움직였는지 봅니다. 값이 클수록 크게 출렁이는 종목이라 손절 여유를 더 두거나 수량을 줄이는 편이 안전합니다.', '긍정: ≤3% 변동 안정 · 일반: ≤6% · 부정: 초과 급등락.'), value: (stock, index) => `${formatTechnicalNumber(technicalNumber(stock, index, 46, 1.5, 5), 2)}%` },
  { key: '52주 신고가 대비', label: '52주 신고가 대비', tooltip: metricTooltip('최근 1년 최고가에서 지금 얼마나 내려와 있는지 봅니다. 0%에 가까울수록 신고가 부근의 강한 종목이고, -30% 이하면 고점에서 크게 밀린 상태입니다.', '긍정: ≥-8% 신고가 근접 · 일반: ≥-25% · 부정: 미만 고점 대비 약세.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 47, -38, 38), 2) + '%' },
  { key: '52주 신고가 후 경과일', label: '신고가 후 경과일', tooltip: metricTooltip('최근 1년 최고가를 찍은 뒤 며칠이 지났는지 봅니다. 짧을수록 최근까지 신고가를 만들던 강한 흐름입니다.', '긍정: ≤20일 최근 강세 · 일반: ≤60일 · 부정: 초과 모멘텀 식음.'), value: (stock, index) => `${Math.round(technicalNumber(stock, index, 48, 1, 180, 0))}일` },
  { key: 'QQQ 대비 상대강도 (20일)', label: 'QQQ 대비 상대강도(20일)', tooltip: metricTooltip('최근 20거래일 동안 이 종목이 나스닥(QQQ)보다 얼마나 더 올랐는지(%p)입니다. +면 시장보다 강함, -면 약함.', '긍정: +2%p↑ 시장 대비 강세 · 일반: ≥-3%p · 부정: 미만 시장 대비 약세.'), value: (stock, index) => formatSignedTechnical(technicalNumber(stock, index, 49, -12, 24), 2) + '%p' },
        ...
        <div className="sheet-wrap value-analysis-sheet" ref={sheetWheelRef}>
          ...
          <thead>
            <tr>
              ...
              <th>
                <MetricValue
                  tooltip={metricTooltip(FAIR_PRICE_RANGE_TOOLTIP, '저평가 긍정, 보통 일반, 고평가 부정.')}
                  onTooltipClose={onTooltipClose}
                  onTooltipOpen={onTooltipOpen}
                >
                  적정 주가 범위
              ...
              <th>
                <MetricValue
                  tooltip={metricTooltip('관심종목의 현재 가격입니다. 적정 주가 범위와 함께 보면 비싼지 싼지 가늠하기 쉽습니다.', '저평가 긍정, 보통 일반, 고평가 부정.')}
                  onTooltipClose={onTooltipClose}
                  onTooltipOpen={onTooltipOpen}
                >
                  현재가
              ...
              <th>
                <MetricValue
                  tooltip={metricTooltip('현재가가 적정 주가 범위보다 아래면 저평가, 안이면 보통, 위면 고평가입니다.', '저평가 긍정, 보통 일반, 고평가 부정.')}
                  onTooltipClose={onTooltipClose}
                  onTooltipOpen={onTooltipOpen}
                >
                  가치 평가
          ...
          <table className="sheet-table value-analysis-table technical-analysis-table">
            <thead>
              <tr>
                ...
                <th>
                  <MetricValue
                    tooltip={metricTooltip('관심종목의 매수·관망·매도 신호입니다. 전략 조건에 따라 자동으로 바뀝니다.', '매수 초록, 관망 일반, 매도 긍정.')}
                    onTooltipClose={onTooltipClose}
                    onTooltipOpen={onTooltipOpen}
                  >
                    투자의견
            ...
            <tr>
              ...
              {groups.map((group) => (
                <th className="event-group-header" colSpan={3} key={group.title}>
                  <MetricValue
                    tooltip={marketEventDisplayTooltip(group.tooltip)}
                    onTooltipClose={onTooltipClose}
                    onTooltipOpen={onTooltipOpen}
                  >
                    {marketEventDisplayLabel(group.title)}
                  </MetricValue>
                </th>
              ))}
            </tr>
...
function App() {
  ...
  async function loadServiceData(session: UserSession | null) {
    try {
      ...
      const activeOperatorTickersFromDb = operatorByTypeFromDb?.[activeType]
      ...
      const nextOperatorByType: Record<InvestmentType, string[]> = operatorByTypeFromDb
        ? {
      ...
      if (!resolvedOperatorWatchlist.clearPending && resolvedOperatorWatchlist.pendingToSync && session && isConfiguredAdminEmail(session.email)) {
        void persistWatchlist('operator', resolvedOperatorWatchlist.pendingToSync, session, nextOperatorByType)
      ...
      // 서버의 유형별 목록(tickers_by_type)을 정본으로 삼는다. 비활성 유형은 서버값을 그대로 보존해
      ...
      // 활성 유형은 위에서 해결한 목록으로, 비활성 유형은 서버 유형별 값으로 채워 유형별 정본을 복원한다.
      if (session) {
        const localByType = readPersonalWatchlistByType(session)
        ...
        // 서버에 유형별 데이터가 전혀 없던 구버전 행이면, 현재 정본을 한 번 올려 다른 기기와 맞춘다.
        if (otherTypeTickersFromDb === null && activePersonalTickersFromDb === undefined && !resolvedPersonalWatchlist.pendingToSync && (legacyTickers || remotePersonalTickers)) {
          ...
          <section aria-modal="true" className="confirm-modal investment-profile-modal" role="dialog">
            ...
            <div className="investment-option-grid">
              {investmentProfileOptions.map((option) => (
                ...
                >
                  <strong className="investment-option-title">
                    <span>{option.title}</span>
                    {option.recommended && <em className="investment-recommended-tag">권장</em>}
                  </strong>
