...
const MARKET_TREND_PATHS = ['web/public/api/market-trends.json', 'data/cache/market-trends.json']
...
async function requireAdmin(req) {
  ...
  if (!readAdminEmails().includes(userEmail)) {
    return { status: 403, error: '관리자만 시장 트렌드를 저장할 수 있습니다.' }
  }
...
async function githubRequest(path, options = {}) {
  ...
  const response = await fetch(`https://api.github.com/repos/${repo}${path}`, {
    ...
    headers: {
      ...
      'user-agent': 'gongsuseongga-market-trends',
...
async function commitMarketTrends(payload) {
  ...
  const entries = await Promise.all(MARKET_TREND_PATHS.map(async (path) => {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!payload || !Array.isArray(payload.rows)) {
      return json(res, 400, { error: 'rows must be an array.' })
    }
    ...
    const commitSha = await commitMarketTrends(saved)
    return json(res, 200, { ...saved, commitSha })
  } catch (error) {
    return json(res, 500, {
      error: error instanceof Error ? error.message : '시장 트렌드 저장에 실패했습니다.',
    })
  }
}
