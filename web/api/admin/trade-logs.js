...
const TRADE_LOG_PATHS = ['web/public/api/trade-logs.json', 'data/cache/trade-logs.json']
...
async function requireAdmin(req) {
  ...
  if (!readAdminEmails().includes(userEmail)) {
    return { status: 403, error: '관리자만 트레이딩 로그를 저장할 수 있습니다.' }
  }
...
async function githubRequest(path, options = {}) {
  ...
  const response = await fetch(`https://api.github.com/repos/${repo}${path}`, {
    ...
    headers: {
      ...
      'user-agent': 'gongsuseongga-trade-logs',
...
async function commitTradeLogs(payload) {
  ...
  const entries = await Promise.all(TRADE_LOG_PATHS.map(async (path) => {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const commitSha = await commitTradeLogs(saved)
    return json(res, 200, { ...saved, commitSha })
  } catch (error) {
    return json(res, 500, {
      error: error instanceof Error ? error.message : '트레이딩 로그 저장에 실패했습니다.',
    })
  }
}
