...
MEGA_TREND_RULES: tuple[tuple[str, tuple[str, ...], tuple[str, ...]], ...] = (
    ("AI 인프라", ("AI 인프라", "AI 서버", "AI 팩토리", "데이터센터", "GPU 클라우드", "GPU 클러스터", "HPC", "액침냉각", "UPS"), ("데이터센터", "전력·냉각", "GPU 클러스터", "HPC", "AI 클라우드")),
    ("AI 반도체", ("AI GPU", "AI칩", "AI 칩", "AI 반도체", "AI 반도체 기판", "CUDA", "HBM", "ASIC", "GPU", "파운드리", "반도체", "첨단패키징", "FC-BGA", "글라스 기판", "CCL", "레이저 어닐링"), ("AI칩", "GPU", "HBM", "ASIC", "첨단패키징")),
    ("우주항공", ("우주", "위성", "발사체", "항공우주", "항공엔진", "LEO", "D2D", "SpaceX", "Rocket Lab", "AST SpaceMobile", "Planet Labs"), ("위성", "발사체", "위성통신", "지구관측", "SpaceX")),
    ("방산·드론", ("AI 방산", "방산", "국방", "드론", "무인기", "무인체계", "유도무기", "레이더", "자율무기"), ("방산", "드론", "무인체계", "유도무기", "레이더")),
    ("전력 인프라", ("전력", "전력망", "전기장비", "가스터빈", "원전", "SMR", "연료전지", "수소", "에너지 저장", "데이터센터 전력", "AI 데이터센터"), ("전력망", "원전", "SMR", "데이터센터 전력", "에너지 저장")),
    ("광통신·네트워크", ("광통신", "광트랜시버", "트랜시버", "광케이블", "광송수신기", "광인터커넥트", "스위칭", "이더넷"), ("광통신", "트랜시버", "광케이블", "AI 네트워킹", "이더넷 스위치")),
    ("로봇·자동화", ("피지컬 AI", "Physical AI", "로봇", "로보틱스", "협동로봇", "자동화", "휴머노이드", "Optimus"), ("로봇", "자동화", "휴머노이드", "협동로봇", "로보틱스")),
    ...
    ("글로벌 K-뷰티", ("K-뷰티", "K뷰티", "글로벌 K-뷰티", "뷰티테크", "홈 뷰티", "화장품", "소셜커머스"), ("K-뷰티", "뷰티테크", "홈 뷰티 디바이스", "글로벌 소비재")),
...
def latest_technical_row(
    ...
    *,
    ...
    if event_watch_active:
        ...
        # 보유 중에도 매수 창이 닫히면 관망이다. 포지션은 유지하되 더 사지 말라는 뜻.
    ...
    strategy_labels = {
        "1": [
            "현재가 < MA200",
            f"VIX >= {float(STRATEGY_RULES['VIX_MIN']):.0f}",
            "RSI < 35 또는 CCI < -150",
            "LR 추세선 상승",
            "저가 추세선 터치",
            downtrend_label,
        ...
        "3": [
            f"정상장 및 QQQ ≤ +{float(STRATEGY_RULES['S3_ENTRY_QQQ_MAX']):.0f}%",
            "현재가 > MA200",
    ...
    return {
        ...
        "볼린저밴드 중단": fmt_price(row["bbMiddle"], stock["market"]),
        "볼린저밴드 하단": fmt_price(row["bbLower"], stock["market"]),
        ...
        "120일 이동평균선": fmt_price(row["ma120"], stock["market"]),
